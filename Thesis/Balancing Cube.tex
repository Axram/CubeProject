...
y_k = Hx_k + z_k
\end{equation}
Where $x$ is the states, in this case the states of the cube which can't be measured directly i.e a position.  The measured value, $y$ (see \eqref{eq:kalmanmeas}) is information from another source, in this case the accelerometer. In an ideal world this would only be a function of $x$, but is distorted by the measurement noise $z$.
The process noise $w$ in equation \eqref{eq:kalmanstate} is a representation of variances in the gyroscope that cannot be mathematically predicted such as flaws in production. The measurement noise, $z$ is common in any measurement and represents various fluctuations caused by the equipment. As the recursive filter uses old and new values a \textit{priori} state $\hat{x}^-_k$  is defined as the estimate of the current state at the time $k$. The \textit{posteriori} state $\hat{x}_k$ is the new estimated state.
...
\end{equation}
As stated above \textbf{is this stated above? ..} the Kalman filter uses readings from both the gyro and accelerometer to estimate a position closer to the true value. A \textit{priori} error covariance matrix is introduced to estimate how much the the readings from the process can be trusted. 
\begin{equation}
P^-_k = AP^-1\textsubscript{k-1}A^T + Q_k
\end{equation}
During the \textit{update} the accelerometer values are used. \textbf{Do i mention earlier that this is the measurement?} The measurement \textit{innovation} is calculated as
\begin{equation}
\tilde{y} = z_k - H\hat{x}^-_k
\end{equation}
This residual reflects the relation between the prediced measurement and the actal measurement. A measurement \textit{innovation} of zero indicates a perfect agreement.
From there the measurement \textit{innovation} covariance is calculated
\begin{equation}
S_k = HP^-_kH^T + R
\end{equation}
This is very similiar to the \textit{priori} error covariance but represents how much the measurement can be trusted. From there the core of the Kalman filter can be calculated, the Kalman gain
\begin{equation}
K_k = P^-1_kH^TS\textsuperscript{-1}_k
\end{equation}
indicates how reliable the measurement is. Not that if the measurement covariance error \eqref{eq:covariance measurement noise} is large the Kalman gain will be small.
...
To improve the Kalman filter the characteristics of the model has to me (ta fram ?? ) . A way of determining the process noise and measurement noise of the IMU is the Allan variance method \ref{AllanV}.
