...
Nu har jag bara slängt ut nyckelord. Den ska styras upp :) Men 2 bra källor iaf tror jag \\
...
algorithm that uses observed measurement over time which in this case is the measurements from the gyro and accelerometer. The problem with the accelerometer is that it's very noisy even tough the cube only is making fine adjustments. The gyro on the other hand, drifts over time \cite{Simon2001}
\cite{Kalmanintro}
